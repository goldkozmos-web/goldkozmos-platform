import { runPlatformSql } from "./applyPlatformSchema";

export const ADMIN_DESK_SQL = `
...
let deskReady: Promise<void> | null = null;

export async function ensureAdminDeskSchema() {
  if (!deskReady) {
    deskReady = runPlatformSql(async (sql) => {
