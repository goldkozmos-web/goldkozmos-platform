...
export function AwarenessPanel() {
  const [items, setItems] = useState<
    { id: string; kind: string; href: string; title: string; summary: string; when: string }[]
  >([]);
  ...
  useEffect(() => {
    ...
    void supabase
      ...
      .select("id, test_kind, result, created_at")
      ...
      .then(({ data }) => {
        setItems(
          (data ?? []).map((row) => {
            ...
            const top = result.top as { id?: string }[] | undefined;
            return {
              id: String(row.id),
              kind,
              href,
              ...
              summary: String(
                result.summary ||
                  result.overall ||
                  result.transform ||
                  (top?.[0]?.id ? `Baskın: ${top[0].id}` : "Kaydedilmiş sonuç"),
              ),
              when: formatWhen(String(row.created_at ?? "")),
            };
          }),
        );
      });
